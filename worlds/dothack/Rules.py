...
def set_monster_rules(location_rules):
    for monster in InfectionMonsters:
        location_rules[monster.name] &= Has(ServerNames[monster.value["server"].name].value)
...
def infection_rules(world):
    ...
    if world.options.monster_hunt:
        set_monster_rules(location_rules)
